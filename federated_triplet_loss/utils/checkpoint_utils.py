...
def get_serial_number(export_dir, prefix="ckpt_"):
  ...
  """
    matcher = re.match(r"^{}(?P<num>\d+)$".format(prefix), os.path.basename(export_dir))
...
def latest_checkpoint(root_output_dir, prefix="ckpt_"):
  ...
  """
    if not tf.io.gfile.exists(root_output_dir):
        return None
    checkpoints = tf.io.gfile.glob(os.path.join(root_output_dir, "{}*".format(prefix)))
    if not checkpoints:
        return None
...
def save(obj, export_dir, prefix=None):
  ...
  """
    if prefix is not None and get_serial_number(export_dir, prefix) < 0:
        raise ValueError(
            'Checkpoint dir "{}" is not named like "{}XXXX!'.format(export_dir, prefix)
        )
