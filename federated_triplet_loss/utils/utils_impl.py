...
def atomic_write_to_csv(
  ...
  """
    ...
    tmp_gfile_name = os.path.join(
        ...
        "{}.tmp{}".format(os.path.basename(output_file), np.random.randint(0, 2 ** 63)),
    )
        ...
        for param in constructor_params:
            ...
            if isinstance(param.default, bool):
                ...
                raise NotImplementedError(
                    ...
                    "optimizers [{!s}]".format(
                        param.name, type(param.default), optimizer_name
                    )
                )
            define_flag_fn(
...
def create_optimizer_from_flags(
    prefix: str, overrides: Optional[Mapping[str, Union[str, float, int, bool]]] = None
    ...
    if optimizer_cls is None:
        ...
        raise ValueError(
            ...
            "one of {!s}. See error log for details.".format(
                optimizer_name, optimizer_flag_name, list(_SUPPORTED_OPTIMIZERS.keys())
            )
        )
...
def remove_unused_flags(prefix, hparam_dict):
  ...
  """
    ...
    if prefixed("optimizer") not in hparam_dict.keys():
        raise ValueError("The flag {!s} was not defined.".format(prefixed("optimizer")))
