  ...
  """
    ...
    def do_eval(round_num):
        ...
        print(
            ...
            "({:.2f} rounds per minute)".format(
                elapsed_minutes, round_num, round_num / elapsed_minutes
            ),
            flush=True,
        )
...
def write_checkpoint(root_checkpoint_dir, server_state, round_num):
    ...
    if tf.io.gfile.exists(root_checkpoint_dir):
        ...
        checkpoints = sorted(
            tf.io.gfile.glob(os.path.join(root_checkpoint_dir, CHECKPOINT_PREFIX + "*"))
        )
    ...
    checkpoint_dir = os.path.join(
        root_checkpoint_dir, "{}{:04d}".format(CHECKPOINT_PREFIX, round_num)
    )
...
def maybe_read_latest_checkpoint(root_checkpoint_dir, server_state):
    ...
    if root_checkpoint_dir is None:
        ...
        logging.info(
            "Looking for checkpoints in [%s/%s].",
            root_checkpoint_dir,
            CHECKPOINT_PREFIX,
        )
    if latest_checkpoint_dir is None:
        ...
        server_state, round_num = read_checkpoint(latest_checkpoint_dir, server_state)
...
def federated_training_loop(
  ...
  """
    ...
    server_state = gan_training_tf_fns.ServerState.from_tff_result(process.initialize())
    logging.info(
    ...
    while round_num < total_rounds:
        ...
        server_state = gan_training_tf_fns.ServerState.from_tff_result(
            process.next(
                server_state,
                server_gen_inputs_fn(round_num),
                client_gen_inputs,
                client_real_inputs,
            )
        )
