...
class ClientOutput(object):
  ...
  """
    ...
    def from_tff_result(cls, anon_tuple):
        ...
        return assert_no_anon_tuples(
            cls(
                discriminator_weights_delta=list(
...
def client_computation(
  ...
  """
    tf.nest.map_structure(
        lambda a, b: a.assign(b), generator.weights, from_server.generator_weights
    )
    tf.nest.map_structure(
    ...
    for gen_inputs, real_data in gen_inputs_and_real_data:
        ...
        num_examples += train_discriminator_fn(
            generator, discriminator, gen_inputs, real_data
        )
    ...
    with tf.control_dependencies([num_examples]):
        weights_delta = tf.nest.map_structure(
            tf.subtract, discriminator.weights, from_server.discriminator_weights
        )
    weights_delta, has_non_finite_delta = tensor_utils.zero_all_if_any_non_finite(
        weights_delta
    )
...
def server_computation(
  ...
  """
    ...
    tf.nest.map_structure(
        lambda a, b: a.assign(b), generator.weights, server_state.generator_weights
    )
    tf.nest.map_structure(
        lambda a, b: a.assign(b),
        discriminator.weights,
    ...
    grads_and_vars = tf.nest.map_structure(
        lambda x, v: (-1.0 * x, v), delta, discriminator.weights
    )
    server_disc_update_optimizer.apply_gradients(grads_and_vars, name="server_update")
    ...
    for gen_inputs in gen_inputs_ds:  # Compiled by autograph.
        gen_examples_this_round += train_generator_fn(
            generator, discriminator, gen_inputs
        )
    ...
    with tf.control_dependencies([gen_examples_this_round]):
        server_state.counters["num_generator_train_examples"] += gen_examples_this_round
...
def create_train_generator_fn(
  ...
  """
    ...
    def train_generator_fn(
        generator: tf.keras.Model, discriminator: tf.keras.Model, generator_inputs
    ):
    ...
    """
        ...
        def gen_loss():
            ...
            return gan_loss_fns.generator_loss(
  ...
  """
    ...
    """
        ...
        def disc_loss():
            ...
            # N.B. The complete pass must be inside loss() for gradient tracing.
            return gan_loss_fns.discriminator_loss(
                generator, discriminator, generator_inputs, real_data
            )

        disc_optimizer.minimize(disc_loss, var_list=discriminator.trainable_variables)
