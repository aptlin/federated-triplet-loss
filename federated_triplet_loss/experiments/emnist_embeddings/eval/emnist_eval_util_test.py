...
class EmnistEvalUtilTest(tf.test.TestCase):
    def setUp(self):
        ...
        images_ds = client_data.create_tf_dataset_for_client(client_data.client_ids[0])
        images_ds = emnist_data_utils.preprocess_img_dataset(images_ds, shuffle=False)
    ...
    def test_emnist_score(self):
        score = eeu.emnist_score(
            self.fake_images, ecm.get_trained_emnist_classifier_model()
        )
        self.assertAllClose(score, 1.1598, rtol=0.0001, atol=0.0001)

        score = eeu.emnist_score(
            self.real_images, ecm.get_trained_emnist_classifier_model()
        )
