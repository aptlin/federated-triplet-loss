...
@functools.lru_cache(maxsize=1)
def get_filtered_by_user_client_data_for_training(
    invert_imagery_probability, accuracy_threshold, batch_size, cache_dir=None
):
  ...
  """
    ...
    try:
        ...
        msg = (
            "A URL fetch failure was encountered when trying to retrieve "
            ...
            "data csv file.".format(invert_imagery_probability, accuracy_threshold)
        )
        raise ValueError(msg)

    return get_filtered_client_data_for_training(
        path_to_read_inversions_csv, None, batch_size
    )


@functools.lru_cache(maxsize=1)
def get_filtered_by_example_client_data_for_training(
    ...
    try:
        ...
        msg = (
            ...
            "data csv files.".format(
                invert_imagery_probability, min_num_examples, example_class_selection
            )
        )
        raise ValueError(msg)
...
def get_filtered_client_data_for_training(
    path_to_read_inversions_csv, path_to_read_example_indices_csv, batch_size
):
    """Form ClientData using paths to pixel inversion, example selection data."""

    raw_client_data = emnist_data_utils.create_real_images_tff_client_data("train")
    ...
    def _get_dataset(client_id):
        ...
        if client_ids_example_indices_map:
            raw_ds = _filter_by_example(
                raw_ds, client_ids_example_indices_map, client_id
            )
...
def _filter_by_example(raw_ds, client_ids_example_indices_map, client_id):
    ...
    return tf.data.Dataset.from_generator(
        _generator, raw_ds.output_types, raw_ds.output_shapes
    )
