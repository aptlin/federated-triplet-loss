...
with utils_impl.record_new_flags() as hparam_flags:
    ...
    flags.DEFINE_integer("noise_dim", 128, "Dimension of the generator input space.")
...
def _create_gen_inputs_dataset(batch_size, noise_dim, seed=None):
    ...
    return (
        ...
        .repeat()
        .map(lambda _: tf.random.normal([batch_size, noise_dim], seed=seed))
    )
...
def _create_real_images_dataset_for_eval():
    ...
    eval_tff_data = emnist_data_utils.create_real_images_tff_client_data(split="test")
...
def _save_images(generator, gen_inputs, outdir, file_prefix):
    ...
    tiled_image = tfgan.eval.python_image_grid(generated_results, grid_shape=(6, 6))

    if not tf.io.gfile.exists(outdir):
        tf.io.gfile.makedirs(outdir)

    f = tf.io.gfile.GFile(os.path.join(outdir, "{}.png".format(file_prefix)), mode="w")
...
def _compute_eval_metrics(
    generator, discriminator, gen_inputs, real_images, gan_loss_fns, emnist_classifier
):
    ...
    disc_loss = gan_loss_fns.discriminator_loss(
        generator, discriminator, gen_inputs, real_images
    )
...
def _get_emnist_eval_hook_fn(
    exp_name,
    output_dir,
    hparams_dict,
    gan_loss_fns,
    gen_input_eval_dataset,
    real_images_eval_dataset,
    rounds_per_save_images,
    ...
    logging.info("Directory %s created (or already exists).", path_to_output_images)
    ...
    def eval_hook(generator, discriminator, server_state, round_num):
        ...
        logging.info("Doing evaluation took %.2f seconds.", time.time() - start_time)
...
def _train(
    ...
    client_real_images_tff_data,
    client_disc_train_steps,
    server_gen_train_steps,
    clients_per_round,
    total_rounds,
    rounds_per_eval,
    eval_hook_fn,
    ...
    def client_datasets_fn(round_num):
        ...
        for client_id in client_ids:
            datasets.append(
                (
                    next(client_gen_inputs_iterator),
                    client_real_images_tff_data.create_tf_dataset_for_client(
                        client_id
                    ).take(client_disc_train_steps),
                )
            )
        return datasets
    ...
    return training_loops.federated_training_loop(
        ...
        root_checkpoint_dir=os.path.join(output_dir, "checkpoints/{}".format(exp_name)),
    )


def _get_path_to_output_image(root_output_dir, exp_name):
    path_to_output_images = os.path.join(root_output_dir, "images/{}".format(exp_name))
...
def main(argv):
    ...
    tff.framework.set_default_executor(
        tff.framework.local_executor_factory(num_clients=FLAGS.num_clients_per_round)
    )
    ...
    if FLAGS.filtering == "by_user":
        client_real_images_train_tff_data = fedu.get_filtered_by_user_client_data_for_training(
        ...
        client_real_images_train_tff_data = fedu.get_filtered_by_example_client_data_for_training(
            invert_imagery_probability=FLAGS.invert_imagery_probability,
        ...
        client_real_images_train_tff_data = fedu.get_unfiltered_client_data_for_training(
            batch_size=CLIENT_TRAIN_BATCH_SIZE
        )

    print(
        "There are %d unique clients that will be used for GAN training."
    ...
    # Eval hook.
    path_to_output_images = _get_path_to_output_image(
        FLAGS.root_output_dir, FLAGS.exp_name
    )
    ...
    eval_hook_fn = _get_emnist_eval_hook_fn(
        ...
        hparam_dict,
        gan_loss_fns,
        gen_inputs_eval_dataset,
        real_images_eval_dataset,
        FLAGS.num_rounds_per_save_images,
        path_to_output_images,
        classifier_model,
    )
