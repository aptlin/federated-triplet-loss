...
def _get_client_ids_and_examples_based_on_classification(
    train_tff_data, min_num_examples, invert_imagery_likelihood, classifier_model
):
...
def main(argv):
    ...
    # Training datasets.
    client_real_images_train_tff_data = emnist_data_utils.create_real_images_tff_client_data(
        "train"
    )

    print(
        "There are %d unique clients."
    ...
    print(
        "There are %d unique clients with at least %d correct examples."
        % (len(client_ids_with_correct_examples_map), FLAGS.min_num_examples)
    )
    print(
        "There are %d unique clients with at least %d incorrect examples."
        % (len(client_ids_with_incorrect_examples_map), FLAGS.min_num_examples)
    )

    # Save client id dictionarys to csv.
    with tf.io.gfile.GFile(
        FLAGS.path_to_save_clients_with_correct_examples_csv, "w"
    ) as csvfile:
    ...
    with tf.io.gfile.GFile(
        FLAGS.path_to_save_clients_with_incorrect_examples_csv, "w"
    ) as csvfile:
        w = csv.writer(csvfile)
    ...
    with tf.io.gfile.GFile(
        FLAGS.path_to_save_correct_example_indices_csv, "w"
    ) as csvfile:
        w = csv.writer(csvfile)
    ...
    with tf.io.gfile.GFile(
        FLAGS.path_to_save_incorrect_example_indices_csv, "w"
    ) as csvfile:
        w = csv.writer(csvfile)
    ...
    print(
        "CSV files with selected Federated EMNIST clients and lists of "
