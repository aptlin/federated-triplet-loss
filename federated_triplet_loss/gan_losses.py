...
class AbstractGanLossFns(abc.ABC):
    ...
    @abc.abstractmethod
    def generator_loss(
        self, generator: tf.keras.Model, discriminator: tf.keras.Model, gen_inputs
    ):
...
def _wass_disc_loss_fn(
    real_images, gen_images, discriminator: tf.keras.Model, grad_penalty_lambda=0.0
):
    ...
    with tf.control_dependencies(
        [tf.assert_equal(tf.shape(real_images)[0], tf.shape(gen_images)[0])]
    ):
        ...
        if grad_penalty_lambda > 0.0:
            disc_loss += _wass_grad_penalty_term(
                real_images, gen_images, discriminator, grad_penalty_lambda
            )
...
def _wass_gen_loss_fn(
    gen_images, discriminator: tf.keras.Model, generator: tf.keras.Model
):
