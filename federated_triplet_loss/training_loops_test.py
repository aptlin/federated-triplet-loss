...
class TrainingLoopsTest(tf.test.TestCase, parameterized.TestCase):
    ...
    @parameterized.named_parameters(
        ("no_dp_and_checkpoint", None, True), ("dp", _get_dp_average_query(), False)
    )
    def test_tff_training_loop(self, dp_average_query, checkpoint):
        ...
        def client_datasets_fn(_):
            return [
                (next(client_gen_inputs), next(client_real_data))
        ...
        if checkpoint:
            ...
            self.assertDictEqual(
                ...
                {
                    "num_rounds": 3,
                    "num_generator_train_examples": 3 * 3 * one_dim_gan.BATCH_SIZE,
                    "num_discriminator_train_examples": (
