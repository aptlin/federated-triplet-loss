...
def tensor_spec_for_batch(dummy_batch):
    ...
    def _get_tensor_spec(tensor):
        ...
        spec = tf.TensorSpec(shape=[None] + tensor.shape.dims[1:], dtype=tensor.dtype)
        return spec
    ...
    def __attrs_post_init__(self):
        ...
        if self.train_discriminator_dp_average_query is not None:
            (
                ...
                from_tff_result_fn=lambda record: list(record),
...
def build_server_computation(
    gan: GanFnsAndTypes, server_state_type: tff.Type, client_output_type: tff.Type
):
  ...
  """
    ...
    @tff.tf_computation(
        ...
        tff.SequenceType(gan.gen_input_type),
        client_output_type,
        gan.dp_averaging_state_type,
    )
    def server_computation(
        server_state, gen_inputs, client_output, new_dp_averaging_state
    ):
...
def build_gan_training_process(gan: GanFnsAndTypes):
  ...
  """
    ...
    server_computation = build_server_computation(
        gan, server_state_type, client_output_type
    )
    ...
    def run_one_round(
        server_state, server_gen_inputs, client_gen_inputs, client_real_data
    ):
        ...
        if gan.dp_averaging_fn is None:
            ...
            (
                ...
                averaged_discriminator_weights_delta,
            ) = gan.dp_averaging_fn(
